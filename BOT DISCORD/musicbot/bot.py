...
class MusicBot(discord.Client):
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                newmsg = '%s - Votre musique **%s** est actuellement jouée dans %s!' % (
                    entry.meta['author'].mention, entry.title, player.voice_client.channel.name)
            else:
                newmsg = 'Actuellement à l écoute dans %s: **%s**' % (
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if 'entries' in info:
            ...
            reply_text = "Ajouté à la file d'attente: **%s** songs . Position dans la file: %s"
            ...
            reply_text = "Ajouté à la file d'attente: **%s** to be played. Position dans la file: %s"
            btext = entry.title
        ...
        if position == 1 and player.is_stopped:
            ...
            try:
                ...
                reply_text += ' - Jouée dans environ: %s'
            except:
                traceback.print_exc()
                time_until = ''
    ...
    async def _cmd_play_playlist_async(self, player, channel, author, permissions, playlist_url, extractor_type):
        ...
        return Response("Ajout de la musique {} songs . Elle sera joué dans {} seconds".format(
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = "Actuellement à l'écoute: **%s** added by **%s** %s\n" % (
    ...
    async def cmd_summon(self, channel, author, voice_channel):
        ...
        if not author.voice_channel:
            raise exceptions.CommandError('Vous êtes hors du channel Musique!')
